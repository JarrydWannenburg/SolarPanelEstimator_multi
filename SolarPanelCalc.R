...
library(lpSolve) # Used for multi-period optimization
...
ui <- fluidPage(
  navbarPage(
    title = "SolarPanelEstimator_multi",
    ...
    tabPanel("Estimations",
             sidebarPanel(titlePanel("Enter Solar Panel Details"),
                          ...
                          numericInput('capital_cost', 'Total System Cost ($)', min=0, max=10000000, value = '', step = 0.0001),
                          ...
                          numericInput('horizon', 'Time horizon', min=0, max=100, value = '', step = 1),
                            bsTooltip('horizon', "How far into the future to estimate. Does not play a role in calculating yearly cost.",'right',options = list(container = 'body')),
                          
                          numericInput('system_life', 'Expected life span (Years)', min=0, max=100, value = '', step = 1),
                            bsTooltip('system_life', "Estimated lifespan of solar system; used to calculate yearly system cost. Decimals are okay.",'right',options = list(container = 'body')),
                          ...
                          #numericInput('tilt','Tilt (degrees)', min=0, max=90, value = 40, step = 0.0001),
                            #bsTooltip('tilt', "Tilt describes the vertical angle of your solar panels.",'right',options = list(container = 'body')),
                          ...
                          #numericInput('gcr','Ground Coverage Ratio', min=0, max=3, value = 0.4, step = 0.0001),
                            #bsTooltip('gcr', "The Ground Coverage Ratio (GCR) is the ratio between the PV modules area and the total ground area.",'right',options = list(container = 'body')),
                          
                          numericInput('inv_eff','Inverter Efficiency (%)', min=90, max=99.99, value = 98, step = 0.01),
                          ...
                          #radioButtons('dataset', 'Dataset Source', choiceNames = c('NSRDB', 'TMY2', 'TYM3','INTL'), choiceValues = c('nsrdb','tmy2','tmy3','intl'),selected = 'nsrdb'),
             ...
             mainPanel(
               fluidRow(
                 column(2,
                        ...
                        bsTooltip('panel_purchase', "The number of panels to install in year one to have the most cost effective setup over the forecast horizon",'bottom',options = list(container = 'body'))),
                 column(2,
...
server <- function(input, output, session) {
  ...
  observeEvent(input$calculate,{
    ...
    tilt <- as.numeric(Loc_values$lat) #Typically just the latitude. May need to change back to input$tilt since lat is pulled from API which is run on calculate button
    ...
    # API Outputs
    station_distance <- round(Solar_Data_Monthly$station_info$distance / 1609.34, 2) #convert meters to miles
...
# #Optimization Estimations (minimize total cost) 
...
#     buy_energy_Q <- 0
#     energy_mix <- c(panels,panels*panel_ac_output,buy_energy_Q) #should panels*panel_ac_output be (panels*panel_ac_output + buy_energy_quantity) so the value would be the max usage of the user? Or should the value shown be the total amount provided by solar, even if that's in excess of what the user requires, as long as it's cheaper to do so? I'd think this situation would be rare
#   } else{
#     #blank space
#   }} else {
#     panels <- 0
...
#     while ((sum(grid_electricity_cost_col) > (capital_cost + installation_cost)) & test_var == TRUE) { #opt_test == TRUE
#           additional_panels = 1
...
# NEW OPTIMIZATION ####
    years <- c(1:ceiling(horizon)) #system_life
    panel_ac_output <- Solar_Data_Monthly$output$ac_annual
    panel_cost_per_year <- (capital_cost + installation_cost) / system_life
    
    # number of periods
    n <- horizon #system_life
...
# OUTPUT CREATION ####
    ...
    yearly_system_spend <- round((opt_num_panels * (capital_cost + installation_cost)) / system_life,0)
      
        #Create the dataframe
      df <- data.frame(years, e_demand, opt_num_panels, yearly_system_spend, yearly_solar_output_col, yearly_levelized_cost_col, e_rate, energy_purchased_col, grid_electricity_cost_col, yearly_solar_savings_col) #round(yearly_system_cost,0)
      colnames(df) <- c('Year', 'Energy Demand (kWh)', 'Optimal Number of Panels', 'Yearly System Cost', 'Yearly Solar Output (kWh)', 'Annual Levelized Cost ($/kWh)', 'Electricity Cost ($/kWh)', 'Energy Purchased (kWh)', 'Grid Electricity Cost ($)', 'Annual Solar Savings ($)')
...
# MAIN TABLE ####
    output$main_df <- renderDataTable({
      DT::datatable(
        ...
        options = list(
          dom = 'tpB',
          lengthMenu = list(c(0, 15, -1), c('0', '15', 'All')),
          pageLength = 15,
          buttons = list(
            list(
              extend = "collection",
          ...
          columnDefs = list(list(className = 'dt-right', targets = "_all")) #columns start from 0
        )
      )
    })
...
# TILE RENDERING ####
    ...
    output$levelized_cost <- renderValueBox({
      valueBox(value = tags$p("Lifetime Levelized Cost:", style = "font-size: 80%;"), 
               subtitle = tags$p(paste0('$', round(sum(yearly_system_cost_col)/sum(yearly_solar_output_col),3)), style = "font-size: 200%;"))
    })
    output$panel_purchase <- renderValueBox({
      valueBox(value = tags$p("Optimal Panels to Purchase: ", style = "font-size: 80%;"), 
               subtitle = tags$p(paste0(max(opt_num_panels)), style = "font-size: 200%;"))
    })
    ...
    output$solar_savings_tile_lifetime <- renderValueBox({
      CF_grid_cost = sum(e_demand * e_rate)
      total_cost = sum(df$`Grid Electricity Cost ($)`) + ( max(results$solution[c(TRUE,FALSE)]) * (capital_cost + installation_cost) )
      valueBox(value = tags$p("Lifetime Savings from solar:", style = "font-size: 80%;"),
               subtitle = tags$p(paste0('$', round(sum(yearly_solar_savings_col),0)), style = "font-size: 200%;"))
    })
    ...
    output$total_cost <- renderValueBox({
      total_cost = sum(df$`Grid Electricity Cost ($)`) + sum(yearly_system_cost_col) #(max(results$solution[c(TRUE,FALSE)]) * (capital_cost + installation_cost))
    ...
    output$total_solar_cost <- renderValueBox({
      total_solar_cost = (mean(opt_num_panels) * (capital_cost + installation_cost)) / system_life
      valueBox(value = tags$p("Average Yearly Optimal System Cost: ", style = "font-size: 80%;"),
               subtitle = tags$p(paste0('$',round(total_solar_cost,0)), style = "font-size: 200%;"))
    })
...
# PLOT RENDERING ####
    ...
    output$annual_levelized_plot <- renderPlot({
      # Visualize the data
      ggplot(df,aes(x = Year)) +
        geom_line(aes(y = `Electricity Cost ($/kWh)`, color = "Electricity Cost ($/kWh)")) + #round(e_rate, 4) ?
